...
public class FacElectronica {
    ...
    private static Log log = LogFactory.getLog(FacElectronica.class);
    ...
    public static String generarXMLZipiadoFactura(String iddocument, Connection conn) {
        ...
        try {
            ...
            log.info("generarXMLZipiadoFactura - RUC " + items.getEmpr_nroruc());
            unidadEnvio = Util.getPathZipFilesEnvio(items.getEmpr_nroruc());
            log.info("generarXMLZipiadoFactura - Ruita de directorios " + unidadEnvio);
            ...
            if (items != null) {
                ...
                resultado = enviarASunat(nrodoc, unidadEnvio, items.getEmpr_nroruc() + "-01-" + items.getDocu_numero() + ".zip", conn, items.getEmpr_nroruc());
                ...
                log.info("generarXMLZipiadoFactura - Proceso envio correo correctamente  " + items.getEmpr_nroruc() + "-01-" + items.getDocu_numero());
...
//        }
    ...
    public static String enviarASunat(String trans, String unidadEnvio, String zipFileName, Connection conn, String vruc) throws com.org.exceptiones.NoExisteWSSunatException {
        ...
        try {
            ...
            //System.out.println("Preparandose a enviar zip y recibir cdr");
            ...
            fos.close();
            //System.out.println("Se creo el cdr y preparandose a para abrir el cdr");
...
//                System.out.println("<<< Archivo: "+file.isFile()+">>>>>" );
    ...
    private Document buildDocument(InputStream inDocument) throws Exception {
        ...
        InputStreamReader reader = new InputStreamReader(inDocument, "ISO8859_1");
    ...
    private static String creaXml(DocumentoBean items, List<DocumentoBean> detdocelec, List<DocumentoBean> anticipos, List<Leyenda> leyendas, String unidadEnvio, Connection conn) {
        ...
        try {
            ...
            if (!items.getDocu_tipo_operacion().equals("-")) {
                Element sUNATTransaction = doc.createElementNS("", "sac:SUNATTransaction");
                envelope.appendChild(sUNATTransaction);
                sUNATTransaction.appendChild(doc.createTextNode("\n"));

                Element ID = doc.createElementNS("", "cbc:ID");
                envelope.appendChild(ID);
                ID.appendChild(doc.createTextNode(items.getDocu_tipo_operacion().trim()));
...
//detalle factura
            ...
            for (DocumentoBean listaDet : detdocelec) {
                ...
                log.info("generarXMLZipiadoFactura - Moneda ");
