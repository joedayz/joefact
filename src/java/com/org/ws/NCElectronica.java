...
public class NCElectronica {

    private static Log log = LogFactory.getLog(NCElectronica.class);

    static {
        org.apache.xml.security.Init.init();
    }

    public static String generarXMLZipiadoNC(String iddocument, Connection conn) {
        log.info("generarXMLZipiadoNC - Inicializamos el ambiente");
        org.apache.xml.security.Init.init();
        String resultado = "";
        ...
        try {
            ...
            log.info("generarXMLZipiadoNC - Extraemos datos para preparar XML ");
            ...
            log.info("generarXMLZipiadoNC - Ruita de directorios " + unidadEnvio);

            //List<DocumentoBean> factura = NCreditoDespachador.cargarNCredito(nrodoc);
            //List<DocumentoBean> detfactura = NCreditoDespachador.cargarDetNCredito(nrodoc);
            if (items != null) {
                ...
                log.info("generarXMLZipiadoNC - Crear Hashcode y CodeBarPDF417");
//                LecturaXML.guardarHashYBarCodePDF417(items.getDocu_tipodocumento(), nrodoc, "DV", pathXMLFile, conn);
                ...
                //======================guardar PDF =======================
                resultado = GeneralFunctions.creaPdf(items, unidadEnvio, conn, "NotaCredito.jasper");
                ...
                if (items.getDocu_enviaws().equals("S")) {
                    log.info("generarXMLZipiadoNC - Preparando para enviar a SUNAT");
                    resultado = enviarASunat(nrodoc, unidadEnvio, items.getEmpr_nroruc() + "-07-" + items.getDocu_numero() + ".zip", conn, items.getEmpr_nroruc());
                } else {
                    /*este caso de boleta no se envia al sunat*/
                    log.info("generarXMLZipiadoNC - no se envia a SUNAT");
                    resultado = "0|El Comprobante numero " + items.getDocu_numero() + ", ha sido aceptado.";
                }
                /*=======================ENVIO CORREO AL CLIENTE=============*/
                resultado = GeneralFunctions.enviarEmail(items, unidadEnvio, conn, resultado, " COMPROBANTE DE NOTA DE CREDITO ELECTRONICA ");
                log.info("generarXMLZipiadoBoleta - Proceso creado correctamente  " + items.getEmpr_nroruc() + "-07-" + items.getDocu_numero());
                LecturaXML.guardarProcesoEstado(nrodoc, "O", resultado.split("\\|", 0), "", conn);
//                resultado="termino de generar el archivo xml de la Nota de Credito";
...
//            Logger.getLogger(NCElectronica.class.getName()).log(Level.SEVERE, null, ex);
//        }
        return resultado;
    }
...
//                System.out.println("<<< Archivo: " + file.isFile() + ">>>>>");
            ...
            System.out.println("==>El envio del Zip a sunat fue exitoso" + zipFileName);
    ...
    private static String creaXml(DocumentoBean items, List<DocumentoBean> detdocelec, List<DocumentoBean> anticipos, List<Leyenda> leyendas, String unidadEnvio, Connection conn) {
        ...
        try {
            ...
            log.info("generarXMLZipiadoNC - Iniciamos cabecera ");
            ...
            log.info("generarXMLZipiadoNC - Lectura de cerificado ");
            CDATASection cdata;
            log.info("generarXMLZipiadoNC - Iniciamos la generacion del XML");
...
//bloque1
            ...
            ResponseCode.appendChild(doc.createTextNode(items.getNota_motivo().trim()));
...
//detalle factura
            log.info("generarXMLZipiadoNC - Iniciamos detalle XML ");
            for (DocumentoBean listaDet : detdocelec) {
                Element CreditNoteLine = doc.createElementNS("", "cac:CreditNoteLine");
                envelope.appendChild(CreditNoteLine);
                CreditNoteLine.appendChild(doc.createTextNode("\n"));

                Element ID11 = doc.createElementNS("", "cbc:ID");
                CreditNoteLine.appendChild(ID11);//se anade al grupo CreditNoteLine
                ID11.appendChild(doc.createTextNode(listaDet.getItem_orden().trim()));

                Element CreditedQuantity = doc.createElementNS("", "cbc:CreditedQuantity");
                CreditedQuantity.setAttributeNS(null, "unitCode", listaDet.getItem_unidad().trim());
                CreditedQuantity.setIdAttributeNS(null, "unitCode", true);

                CreditNoteLine.appendChild(CreditedQuantity);//se anade al grupo CreditNoteLine
                CreditedQuantity.appendChild(doc.createTextNode(listaDet.getItem_cantidad().trim()));
                ...
                CreditNoteLine.appendChild(LineExtensionAmount1);//se anade al grupo CreditNoteLine
                ...
                CreditNoteLine.appendChild(PricingReference);//se anade al grupo CreditNoteLine
                ...
                CreditNoteLine.appendChild(TaxTotal1);//se anade al grupo CreditNoteLine
                ...
                CreditNoteLine.appendChild(Item);//se anade al grupo CreditNoteLine
                ...
                CreditNoteLine.appendChild(Price);//se anade al grupo CreditNoteLine
            ...
            {
                //Firmar el documento
                log.info("generarXMLZipiadoNC - firma el XML ");
                sig.sign(privateKey);
            }
            ...
            log.info("generarXMLZipiadoNC - XML creado " + pathXMLFile);
